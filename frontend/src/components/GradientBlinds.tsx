...
const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  // Compute effective stops taking into account color1 and color2 overrides
  const effectiveStops = gradientColors ?? (color1 && color2 ? [color1, color2] : undefined);
...
`;
    ...
    const { arr: colorArr, count: colorCount } = prepStops(effectiveStops);
    const uniforms: {
